...
class MessageListState {
  ...
  /** 是否有更早的消息 */
  public readonly hasMoreOlderMessage: { value: boolean };
  
  /** 是否有更新的消息 */
  public readonly hasMoreNewerMessage: { value: boolean };
  ...
  private constructor(conversationID: string, messageListType: MessageListType) {
    ...
    this.hasMoreOlderMessage = makeReactive({ value: true });
    this.hasMoreNewerMessage = makeReactive({ value: false });
  ...
  private handleC2CCallSignaling(messageList: MessageInfo[]): MessageInfo[] {
    ...
    for (const message of messageList) {
      ...
      const customData = (message.messageBody && message.messageBody.customMessage && message.messageBody.customMessage.data) ? message.messageBody.customMessage.data : undefined;
      if (!customData) {
        result.push(message);
        continue;
      }
      ...
      try {
        ...
        const isExcludedFromUnreadCount = rawMessage ? (rawMessage._isExcludedFromUnreadCount != null ? rawMessage._isExcludedFromUnreadCount : rawMessage.isExcludedFromUnreadCount) : undefined;
        const isExcludedFromLastMessage = rawMessage ? (rawMessage._isExcludedFromLastMessage != null ? rawMessage._isExcludedFromLastMessage : rawMessage.isExcludedFromLastMessage) : undefined;
