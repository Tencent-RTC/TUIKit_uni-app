...
export const parseMessageToRichTextNodes = (message: MessageInfo): RichTextNode[] => {
  if (message.messageType === MessageType.TEXT && (message.messageBody && message.messageBody.text)) {
...
const getMessageAbstract = (message: MessageInfo): string => {
  ...
  switch (message.messageType) {
    case MessageType.TEXT:
      messageContent = (messageBody && messageBody.text) || '[文本消息]'
      break;
    ...
    case MessageType.FILE:
      messageContent =`[文件] ${(messageBody && messageBody.fileName) || ''}`
      break;
    ...
    case MessageType.CUSTOM:
      ...
      if (isCallMessage(message)) {
        ...
        messageContent = (messageBody && messageBody.customMessage && messageBody.customMessage.description) || '[自定义消息]'
      }
      break;
      
    case MessageType.MERGED:
      messageContent = '[聊天记录] ' + ((messageBody && messageBody.mergedMessage && messageBody.mergedMessage.title) ? messageBody.mergedMessage.title : '')
      break;
