...
export function usePromoteAdmin(routeParams?: any): UserPickerHookResult {
  ...
  const { groupMemberList: allMembers, hasMoreGroupMembers, setGroupMemberRole, fetchMoreGroupMemberList } = useGroupMemberState({ groupID })
  const { fetchGroupMemberList } = useGroupMemberState({ groupID, role: GroupMemberRole.Admin })
  ...
  const handleConfirm = async (selectedUsers: User[]): Promise<void> => {
    ...
    try {
      for (const user of selectedUsers) { await setGroupMemberRole(user.userID, GroupMemberRole.Admin) }
  ...
  const onReachEnd = async (): Promise<void> => {
    ...
    try { await fetchMoreGroupMemberList() } catch (error) { console.error('[usePromoteAdmin] onReachEnd failed:', error) }
  }
