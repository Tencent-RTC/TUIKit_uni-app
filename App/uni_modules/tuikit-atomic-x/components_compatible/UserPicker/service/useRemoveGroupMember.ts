...
export function useRemoveGroupMember(routeParams?: any): UserPickerHookResult {
  ...
  const { groupMemberList: allMembers, hasMoreGroupMembers, deleteGroupMember, fetchMoreGroupMemberList } = useGroupMemberState({ groupID })
  ...
  const handleConfirm = async (selectedUsers: User[]): Promise<void> => {
    ...
    try {
      await deleteGroupMember(selectedUsers.map(user => user.userID))
  ...
  const onReachEnd = async (): Promise<void> => {
    ...
    try { await fetchMoreGroupMemberList() } catch (error) { console.error('[useRemoveGroupMember] onReachEnd failed:', error) }
  }
