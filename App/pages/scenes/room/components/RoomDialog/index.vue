<template>
  ...
  >
    <div class="dialog-container" @tap.stop>
      <text class="dialog-title">{{ props.title }}</text>
      <div :class="[hasTitle ? 'dialog-content' : 'dialog-content-notitle']">
        <slot></slot>
      </div>
      <div class="dialog-footer">
        <text v-if="cancelButton" class="cancel-button" @tap="handleCancel">{{ props.cancelButton }}</text>
        <text class="confirm-button" :class="{ 'confirm-button-only': !cancelButton }" @tap="handleConfirm">{{ props.confirmButton }}</text>
      </div>
    </div>
  </div>
</template>
