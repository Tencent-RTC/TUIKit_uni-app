...
export default class TUIChatKit {
  ...
  public login() {
    ...
    this.chat = chat;
    // Forward login success to TUIBridge so that CallKit (MP-WEIXIN) can
    // receive the event and initialize.
    TUIBridge.notifyEvent({
      eventName: EVENT.LOGIN_SUCCESS,
      params: { chat, SDKAppID, userID, userSig },
    });
