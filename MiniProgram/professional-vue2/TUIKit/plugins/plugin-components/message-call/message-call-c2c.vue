<template>
  <div v-if="isCallMessage && conversationType === TYPES.CONV_C2C" class="call" :class="['call-' + conversationType, message.flow === 'out' && 'call-reverse']" @click="callAgain">
...
<script setup lang="ts">
  ...
  watch(
    () => props.customContent,
    (newVal) => {
      // 只接受有值的 custom，避免后续空对象覆盖
      if (newVal?.custom) {
        custom.value = newVal.custom;
      }
    },
  ...
  const callAgain = () => {
    if (conversationType.value === TUIChatEngine.TYPES.CONV_C2C) {
      ...
      // Use TUICore.callService for App environment (native plugin);
      // fall back to TUIBridge for MP-WEIXIN CallKit integration.
      if (!isCallIntegrated()) {
        TUICore.callService({
