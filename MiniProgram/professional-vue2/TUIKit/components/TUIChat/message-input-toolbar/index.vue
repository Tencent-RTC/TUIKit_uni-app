...
const getExtensionList = () => {
  ...
  // In MP-WEIXIN environment, the native CallkitPluginServer is not initialized.
  // When TUIBridge-based CallKit is integrated, inject call extension buttons manually.
  const hasVoiceCall = currentExtensionList.value.some(ext => ext?.data?.name === 'voiceCall');
  const hasVideoCall = currentExtensionList.value.some(ext => ext?.data?.name === 'videoCall');
  if (!hasVoiceCall && !hasVideoCall && isCallIntegrated()) {
    if (chatType !== TUIConstants.TUIChat.TYPE.CUSTOMER_SERVICE) {
      currentExtensionList.value.push(
        {
          weight: 1000,
          text: '语音通话',
...
const onCallExtensionClicked = (extension: ExtensionInfo, callType: number) => {
  ...
  if (currentConversation?.value?.type === TUIChatEngine.TYPES.CONV_C2C) {
    const userIDList = [currentConversation?.value?.conversationID?.slice(3)];
    // If the extension has a native listener (App environment), use it directly.
    // Otherwise fall back to TUIBridge for MP-WEIXIN CallKit integration.
    if (extension?.listener?.onClicked) {
...
const onUserSelectorSubmit = (selectedInfo: any) => {
  // If the extension has a native listener (App environment), use it directly.
  // Otherwise fall back to TUIBridge for MP-WEIXIN CallKit integration.
  if (currentUserSelectorExtension.value?.listener?.onClicked) {
    ...
    TUIBridge.notifyEvent({
      eventName: EVENT.ON_CALLS,
      params: {
        userIDList: selectedInfo.userIDList,
