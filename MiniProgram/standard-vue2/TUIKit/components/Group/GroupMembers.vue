<template>
  <view class="group-members">
    ...
    <view class="members-grid">
      ...
      <!-- 添加成员按钮 -->
      <view v-if="!readonly" class="member-item add-member" @click="onAddMember">
        <view class="member-avatar-container">
      ...
      <!-- 删除成员按钮 (仅群主可见) -->
      <view 
        v-if="!readonly && currentUserRole === GroupMemberRole.OWNER" 
    ...
    <!-- 折叠/展开按钮 -->
    <view 
      v-if="!readonly && totalMembers > displayLimit" 
      class="toggle-button"
...
export default {
  ...
  computed: {
    // 状态数据
    allMembers() {
      return useGroupSettingState().allMembers;
    ...
    getGroupMemberList() {
      const groupSettingState = useGroupSettingState();
      return groupSettingState.getGroupMemberList;
    },
    ...
    displayMembers() {
      ...
      return members.slice(0, this.displayLimit);
    },
    
    removableMembers() {
      // 过滤掉群主
      return this.allMembers?.filter(member => member.role !== GroupMemberRole.OWNER) || [];
    },
    
    currentMemberIDs() {
      return this.allMembers?.map(member => member.userID) || [];
    }
  },
  async mounted() {
    try {
  ...
  methods: {
    ...
    async handleAddMembers(userIDs) {
      try {
        await this.$emit('addMembers', userIDs);
    ...
    async handleRemoveMembers(userIDs) {
      try {
        await this.$emit('removeMembers', userIDs);
